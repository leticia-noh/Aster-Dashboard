...
const DevolutivaFeedbackFormSchema = z.object({
    ...
    avaliacao: z.string().refine((value) => Number(value) >= 0, {
...
export type DevolutivaFeedbackFormSchemaType = z.infer<typeof DevolutivaFeedbackFormSchema>

type devolutivaFeedbackProps = {
    devolutivaFeedback?: DevolutivaFeedback
}
...
export default function DevolutivaFeedbackForm({ devolutivaFeedback }: devolutivaFeedbackProps) {
    ...
    // useForm
    const methods = useForm<DevolutivaFeedbackFormSchemaType>({
        mode: 'all',
        resolver: zodResolver(DevolutivaFeedbackFormSchema),
        defaultValues,
        values: devolutivaFeedback && {
            id: devolutivaFeedback.id,
            avaliacao: devolutivaFeedback.avaliacao,
            atividadeUso: devolutivaFeedback.atividadeUso,
            dataEnvio: devolutivaFeedback.dataEnvio,
            assunto: devolutivaFeedback.assunto,
            produtoId: devolutivaFeedback.produtoId,
            clienteDocumento: devolutivaFeedback.clienteDocumento,
            mensagem: devolutivaFeedback.mensagem
        }
    })
    ...
    const handleCreateEdit: SubmitHandler<DevolutivaFeedbackFormSchemaType> = (async (data) => {
        try {
            if (devolutivaFeedback) {
                // Hook de edit
                await EditarDevolutivaFeedback({ ...data, dataEnvio: dayjs(data.dataEnvio).format('YYYY-MM-DD') })
